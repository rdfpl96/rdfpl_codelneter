...
class Product_model extends CI_Model{
  function __construct(){
   ...
   public function getSimillerProduct($start,$records_per_page,$top_cat_id,$sub_id,$child_cat_id){
        $array_data=array();
        $this->db->select([
          'P.product_name',
          'P.product_id',
...
$this->db->join('tbl_sub_category AS SC', 'PWM.sub_cat_id = SC.sub_cat_id');
$this->db->join('tbl_child_category AS CC', 'PWM.child_cat_id = CC.child_cat_id');
$this->db->join('tbl_product_variants AS V', 'V.variants_product_id = P.product_id', 'left');
...
if ($shortBy !== "") {
    $this->db->order_by('V.price', $shortBy);
}

if ($filterbyPrice !== "") {
    $priceArray = explode(',', $filterbyPrice);
     $this->db->where('V.price >=', $priceArray[0]);
    $this->db->where('V.price <=', $priceArray[1]);
}
